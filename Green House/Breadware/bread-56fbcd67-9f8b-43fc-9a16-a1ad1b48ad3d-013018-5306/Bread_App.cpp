...
unsigned int num_sensor_keys = 3;
String BREAD_APP_allDataSensorKeys[3] = {"TMP_T", "HUM_H", "BRI_L"};
String BREAD_APP_getSensorReading(String dataKey) {
    String val = "";
    if (dataKey == "TMP_T") {
        val = String(temp1.read());
    }
    if (dataKey == "HUM_H") {
        val = String(humidity1.read());
    }
    if (dataKey == "BRI_L") {
        val = String(ldr1.read());
    }
    return val;
}
